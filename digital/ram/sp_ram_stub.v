...
module SP (
    ...
    generate
        if (BIT_WIDTH == 32) begin
            assign addr = AD[ADDR_BITS-1+2:2];  // AD[10:2] for 512 depth
        end else if (BIT_WIDTH == 16) begin
            assign addr = AD[ADDR_BITS-1+4:4];  // AD[13:4] for 1024 depth
        end else if (BIT_WIDTH == 8) begin
            assign addr = AD[ADDR_BITS-1+5:5];  // AD[15:5] for 2048 depth (but AD is only 14-bit)
        end else begin
